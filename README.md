...
Paste a `bit.ly`, `lnkd.in`, `t.co`, or any other shortened link, and Tiny URL EmBiginator walks the redirect chain in the background, shows you the final destination, and (optionally) checks the destination's certificate history and domain registration so you can decide whether to trust it before clicking.
...
- **Right-click to investigate any link.** Right-click a hyperlink on any page and choose *"Resolve with Tiny URL EmBiginator"* — the popup opens with the URL pre-filled and resolved automatically.
...
Install Tiny URL EmBiginator from its Chrome Web Store listing. Once installed, pin the extension from the toolbar puzzle-piece menu for one-click access.
...
2. **Right-click any link** on any page and choose *"Resolve with Tiny URL EmBiginator"*. The popup opens and resolves the link automatically.
...
Tiny URL EmBiginator declares the minimum permissions needed to do its job:
...
| `contextMenus` | Adds the *"Resolve with Tiny URL EmBiginator"* item to the right-click menu when you right-click a link. |
...
- **HTTP redirects only.** Tiny URL EmBiginator follows `3xx` responses with `Location` headers. It does not execute JavaScript, so JS-driven redirects are out of scope.
...
Tiny URL EmBiginator is built around the principle of *not* opening sketchy links. It does not collect personal data, has no analytics, and sends no telemetry to the developer. Resolved-link history is stored locally and never leaves your device.
